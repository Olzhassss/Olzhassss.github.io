...
<section>
	<div class="mx-auto container cursor-default">
		<h2 class="mb-4 font-semibold">
			Who am I? <span class="text-slate-300 text-sm">I mean...</span>
		</h2>
...
<section>
	<div class="mx-auto container cursor-default">
		...
		<div class="h-48 w-full relative">
			...
			>
				<button
					class="w-24 h-24 rounded-full border border-white hover:border-cyan-400 transition-colors
				...
				>
					<div
						class="w-6 h-6 border border-white group-hover:border-cyan-400 transition-colors animate-ping rotate-90"
					/>
				</button>
			</div>
			{#each lines as line}
				<pre
					class="text-3xl ml-12 transition-opacity whitespace-pre-wrap {line['show']
						? 'opacity-100'
						: 'opacity-0'}">{line['text']}</pre>
			{/each}
		</div>
	</div>
</section>
